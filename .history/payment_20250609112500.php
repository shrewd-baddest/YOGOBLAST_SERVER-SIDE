...
if (!$checkout_id) {
    echo json_encode([
        "status" => "error",
        "message" => "No CheckoutRequestID in response.",
        "response" => $response
    ]);
    curl_close($ch);
    mysqli_close($conn);
...
file_put_contents("callback_log.json", json_encode($response) . PHP_EOL, FILE_APPEND);
...
mysqli_stmt_bind_param($stmt, 'sidsi', $checkout_id, $user_id, $amount, date('Y-m-d H:i:s'), $id);
