...
$transacts="SELECT * FROM mpesa_request WHERE checkout_id=? LIMIT=1";
$stm4=mysqli_prepare($conn,$transacts);
mysqli_stmt_bond_param($stm4,'s',$checkoutID);
mysqli_stmt_execute($stm4);
$results=mysqli_stmt_get_result($stm4);
$rows = mysqli_fetch_assoc($results);

// Store the fetched row for testing
file_put_contents('callback_rows_log.json', json_encode($rows, JSON_PRETTY_PRINT) . PHP_EOL, FILE_APPEND);
